...
const updateMdatLocation = (mdatLocations, lastMdatLocation, currentOffset) => {
  if (lastMdatLocation) {
...
//           rudimentary track objects later
//
// Limitations:
...
const mdatMapper = (inFd, useHEAAC = false) => {
  ...
  while (true) {
    ...
    if (probe[0] === 0xFF && (probe[1] === 0xF1 || probe[1] === 0xF0)) {
      ...
      // We are no longer updating the lastMdatLocation
      lastMdatLocation = null;
...
// In other words, it groups NAL Units into single samples)
